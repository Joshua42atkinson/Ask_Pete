...
use crate::authoring::blueprint_station::BlueprintStation;
use crate::authoring::common_ui::Toast;
use crate::authoring::owl_diagnostic::OwlDiagnostic;
use crate::authoring::story_node::StoryNodeComponent;
...
pub fn NodeCanvas() -> impl IntoView {
    ...
    view! {
        <div class="relative w-full h-full bg-industrial-slate blueprint-grid"
        ...
        >
            ...
            <div class="absolute top-0 left-0 right-0 z-20">
                ...
                <div class="bg-slate-900 border-b border-slate-700 px-4 py-1.5 flex items-center gap-1 text-sm text-slate-300">
                    ...
                    <button
                        class="px-3 py-1 bg-slate-800 text-slate-300 font-bold rounded hover:bg-slate-700 transition-colors flex items-center gap-2 mr-4 border border-slate-600"
                        on:click=move |_| set_view_transform.set((0.0, 0.0, 1.0))
                        title="Reset Camera View"
                    >
                        <span>"👁️ Reset View"</span>
                    </button>
            ...
            {move || selected_node_id.get().and_then(|id| {
                nodes.get().iter().find(|n| n.get().id == id).cloned().map(|node_signal| {
                    view! {
                        <crate::authoring::inspector::Inspector
                            node=node_signal
            ...
            {move || if show_word_smithy.get() {
                view! {
                    <WordSmithy
                        ...
                        on_save=Callback::new(move |def: crate::authoring::word_smithy::WordDefinition| {
