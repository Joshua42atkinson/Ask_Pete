...
pub fn sync_ecs_to_shared(
    ...
    shared_log: Res<crate::components::SharedResearchLogResource>,
    shared_virtues: Res<crate::components::SharedVirtuesResource>,
) {
...
pub struct DownloadChannel(
    pub tokio::sync::mpsc::Receiver<infra_services::downloader::DownloadProgress>,
);
...
pub fn download_manager_system(
    ...
    for event in events.read() {
        ...
        tokio::spawn(async move {
            if let Err(e) = infra_services::downloader::download_file(full_url, path, tx).await {
...
pub fn sync_inbox_to_events(
    inbox: Res<crate::components::DownloadCommandInbox>,
...
pub fn progress_update_system(
    ...
    shared_state: Res<crate::components::SharedDownloadStateResource>,
) {
...
pub fn sync_physics_to_shared(
    ...
    shared_physics: Res<crate::components::SharedPhysicsResource>,
) {
...
pub fn sync_campaign_to_shared(
    current_state: Res<crate::multiplayer_client::CampaignState>,
...
// [NEW] System to sync Story Progress to Shared Resource (Bevy -> Axum)
pub fn sync_story_progress_to_shared(
    query: Query<&StoryProgress, Changed<StoryProgress>>,
    shared_progress: Res<crate::components::SharedStoryProgressResource>,
) {
    if let Ok(progress) = query.get_single() {
        if let Ok(mut guard) = shared_progress.0.write() {
...
pub fn sync_vote_inbox(
    inbox: Res<crate::components::VoteInbox>,
    mut event_writer: EventWriter<crate::components::VoteEvent>,
) {
    if let Ok(mut guard) = inbox.0.write() {
        for event in guard.drain(..) {
            event_writer.send(event);
...
pub fn sync_quest_inbox(
    inbox: Res<crate::components::QuestCommandInbox>,
    mut event_writer: EventWriter<crate::components::StartQuestEvent>,
) {
    if let Ok(mut guard) = inbox.0.write() {
        for event in guard.drain(..) {
            event_writer.send(event);
...
pub fn handle_start_quest(mut events: EventReader<crate::components::StartQuestEvent>) {
    for event in events.read() {
        info!("Starting Quest: {}", event.quest_id);
