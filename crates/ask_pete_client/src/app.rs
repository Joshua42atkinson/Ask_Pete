...
pub fn App() -> impl IntoView {
    ...
    // [NEW] Bevy Initialization & Telemetry Polling
    Effect::new(move |_| {
        // 1. Start Bevy (Once)
        // We use a timeout to ensure the canvas is in the DOM
        let _ = gloo_timers::callback::Timeout::new(100, move || {
            crate::bevy_app::run_bevy_app("bevy-canvas".to_string());
        })
        .forget();

        // 2. Poll Telemetry (Every 100ms)
        let handle = Interval::new(100, move || {
            if let Ok(state) = PHYSICS_STATE.read() {
                set_coal.set(state.coal);
                set_steam.set(state.steam);
                // set_velocity.set(state.velocity); // If we had a velocity signal

                // Only update GPS if not manual
                if !is_manual_location.get_untracked() {
                    // In a real app, we might mix GPS and Physics, but for now Physics overrides
                    // set_latitude.set(state.latitude);
                    // set_longitude.set(state.longitude);
                }
            }
        });

        // Leak the interval so it keeps running (for this demo)
        handle.forget();
    });
